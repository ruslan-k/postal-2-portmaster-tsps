...
- Sep 26 with force-visible cursor: KMS screenshot shows the cursor on the options screen; SDL logs show `ShowCursor requested=0 effective=1`, with motion events delivered. X11 root is 1280×720, but the focused SDL child is 640×480 at +0,+0 and `SDL_WM_GrabInput(ON)` is active; live X pointer reached `(639,451)`, the child-window edge. This geometry/grab mismatch explains confinement to the root's top-left 640×480 area.
- Next isolated geometry A/B: Xorg dummy root mode changes 1280×720→640×480 to match the unchanged 640×480 game window, while the separate presenter/KMS output remains 1280×720. This should make the SDL window cover its entire input root without changing game resolution, mouse events, or cursor visibility. KMS output size and on-screen cursor range need the next physical test; no visual fix claimed yet.
