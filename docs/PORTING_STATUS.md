...
- Sep 26 relative-mode physical A/B: cursor becomes visible but is confined to upper-left region (user). Fresh SDL event sequence starts `xrel/yrel=256,192`, which the interposer turned into `0,0`; the next position `383,168` became only `127,-24`. Live Xorg root is 1280x720 but the SDL child is 640x480 at +0,+0 and its X11 pointer reaches (639,479). A KMS capture is 1280x720 with 4:3 content letterboxed (approximately x=160..1119), so the renderer remains 640x480. The first omitted 256x192 cursor offset is a concrete explanation for a logical cursor range limited to the upper-left portion; visible full-range behavior is not yet proven.
- Next one-variable input A/B: seed first relative mouse event with its pointer position, then keep the proven consecutive deltas; preserve absolute/button events, 640x480 game renderer, Xorg and presenter. Test the bottom-right menu and A click from the physical menu.
- 720p: the game config has independent Windowed/Fullscreen/MenuViewport dimensions, but the live game reports `SetRes: 640x480`, and the current presenter source is 640x480 on a 1280x720 KMS target. A previous 1280x720 window-only experiment cropped the UI. A coherent 1280x720 guest+presenter profile is possible to stage, but native TSPS fullscreen correctness/performance are unverified; do not combine that graphics A/B with this cursor-offset A/B.
