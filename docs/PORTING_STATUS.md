...
- Correction deployed from commit `1c88ace`: relative mode now passes SDL 1.2 x/y and xrel/yrel through unchanged. Host tests verify a native delta sequence accumulates the full 639×479 range; device SHA-256 matches local. Physical menu verification is pending. Absolute mode remains the known failed optional A/B.
