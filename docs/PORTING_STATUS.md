...
- Next one-variable A/B: on the next physical menu launch, `POSTAL2_MOUSE_COORD_MODE=absolute` replaces only mouse-motion and button absolute `x/y` with the observed pointer coordinates; relative `xrel/yrel`, keyboard, renderer, and map remain unchanged. The i386 interposer logs raw and normalized coordinates. `POSTAL2_MOUSE_COORD_MODE=passive` restores no-op observation. This is a diagnostic candidate, not a verified input fix.
