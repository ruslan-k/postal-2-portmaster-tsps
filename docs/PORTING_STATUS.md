...
- In that run, unmodified SDL motion events reached `(639,479)` and left-button down/up were logged at `(639,479)`, the full 640×480 guest boundary. This proves delivery to the SDL boundary, not that Unreal's UWindow hover/click state or the visible cursor maps to the same coordinates. Next physical A/B changes only `SDL_WM_GrabInput(SDL_GRAB_ON)` to OFF (`POSTAL2_FORCE_UNGRAB=1`); it preserves raw SDL motion/button fields and force-visible cursor. The launcher logs requested/effective grab modes; `POSTAL2_FORCE_UNGRAB=0` is the unchanged comparator. If that does not affect the physical cursor range, proceed to instrument the Unreal mouse-axis accumulator/viewport and pointer-vs-game coordinate transform rather than repeating root-size or guessed scaling A/Bs. KMS output stayed 1280×720. Exact Postal 2 process was stopped after log/screenshot capture; gptokeyb2, Xorg, and presenter exited, MainUI returned.
