...
- Sep 26 cursor-visibility probe: guest called `SDL_ShowCursor(toggle=0)` (previous state/result `1`, i.e. it requested hide), then `SDL_WM_GrabInput` modes `0` and later `1`. The KMS frame had no visible cursor while SDL motion events were reaching the guest. This is a strong hide/grab lead; it is not yet proved that SDL's host cursor is the Unreal menu cursor.
- Next reversible A/B: preserve mouse event fields and grab behavior; override only guest `SDL_ShowCursor(SDL_DISABLE)` to `SDL_ENABLE` when `POSTAL2_FORCE_CURSOR=1` (default for this test), log requested/effective toggle, and leave `POSTAL2_FORCE_CURSOR=0` as no-override comparator. Host tests pass; physical visible-cursor/menu result pending. Latest game PID 16993 was stopped after capture; TERM was ignored, then KILL was sent only to that exact verified PID. Port processes exited and MainUI returned.
