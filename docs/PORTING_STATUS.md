...
- Next isolated A/B: set `POSTAL2_MOUSE_COORD_MODE=relative`. Preserve raw x/y and button events; turn the observed pointer-like xrel/yrel into per-event deltas. Avoid double conversion when `SDL_PollEvent` internally calls `SDL_PeepEvents`. Preserve the prior binary and launcher as a named backup. This remains a hypothesis pending physical menu proof.
