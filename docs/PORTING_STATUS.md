...
- Sep 26 relative-mode A/B with the seeded first delta did not fix the user's upper-left cursor limit. Fresh launcher logs show `x/y=639,479` while the physical-position-like `xrel/yrel` changes: first `(256,192)`, next `(383,168)`, later `(639,361..479)`. The shim's normalized deltas therefore accumulate across the full 639×479 range in its own host test, but this was not sufficient for the visible Unreal cursor. The game remains 640×480 (`SetRes: 640x480`) in a 1280×720 Xorg/KMS path; prior absolute x/y rewrite also failed and made the cursor disappear. The first-offset diagnosis was incomplete; stop treating it as a fix.
- Current physical run is still live (postal2-bin 11832, Xorg 11814, gptokeyb2 11778, presenter 11764 at the last check); do not replace loaded artifacts or stop it. Next diagnostic must distinguish SDL motion consumption from Unreal's absolute `WindowsMouseX/Y`/UWindow cursor clamp, using a fresh bounded raw/post-transform event trace and the actual menu frame. Do not apply another coordinate mapping or 720p change until that boundary is identified. Ask the user to exit the current run before deploying a new guest library.
