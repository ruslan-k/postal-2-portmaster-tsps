...
- Sep 26 ungrab A/B: launcher logged `force_ungrab=1`; the game requested `SDL_WM_GrabInput(1)` and the probe passed effective mode `0` (SDL result `0`). The user reported the cursor no longer moved. KMS frame shows the visible pointer at the extreme upper-left of the presented picture; meanwhile SDL motion records vary from `(256,192)` through `(496,342)`. Therefore disabling SDL grab breaks visible pointer tracking despite guest event delivery. This A/B is rejected. Launcher default is restored to `POSTAL2_FORCE_UNGRAB=0`; the override remains opt-in only. Next isolate the mismatch between X11-visible pointer and Unreal's cursor/viewport while preserving grab; do not repeat root-size or guessed scaling A/Bs. Exact Postal 2 PID was stopped; gptokeyb2/Xorg/presenter exited and MainUI returned. KMS output stayed 1280×720.
